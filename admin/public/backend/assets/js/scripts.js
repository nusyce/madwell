...
// ===== GLOBAL REFS =====
let map, partnerMap;
let marker, autocomplete, addPartnerLocation;

// ===== MAPS CALLBACK =====
// function initMapsOnly() {
//     initMap();
//     initPartnerMap();
// }
// window.initMapsOnly = initMapsOnly;

// // ===== PLACES CALLBACK =====
// function initPlacesOnly() {
//     initAutocomplete();
//     initPartnerAutocomplete();
// }
// window.initPlacesOnly = initPlacesOnly;
...
function updateLocationFromCoordinates(lat, lng) {
    ...
    // Check if Google Maps Geocoding API is available
    if (typeof google !== 'undefined' && google.maps && google.maps.Geocoder) {
        ...
        geocoder.geocode({ location: latlng }, function(results, status) {
            if (status === 'OK' && results[0]) {
                // Successfully got address - update the location input with formatted address
                const address = results[0].formatted_address;
                locationInput.value = address;
            } else {
                // Geocoding failed - fallback to showing coordinates
                console.log('Geocoding failed:', status);
                locationInput.value = `${lat}, ${lng}`;
            }
        });
    } else {
        // Google Maps not available - fallback to showing coordinates
        locationInput.value = `${lat}, ${lng}`;
    }
}
...
function initPartnerMap() {
    ...
    // Check if latitude and longitude are already set in form fields
    const existingLat = parseFloat($("#partner_latitude").val());
    const existingLng = parseFloat($("#partner_longitude").val());
...
function setupPartnerMapListeners() {
    ...
    partnerMap.addListener("click", (e) => {
        ...
        updateLocationFromCoordinates(newLat, newLng);
    });
    ...
    // Note: Event listeners are now set up in setupPartnerMapListeners() function
    // This avoids code duplication and makes the code cleaner
    // console.log('Partner map initialized with marker at:', lat, lng);
}
...
function prefillLocationFromCoordinates() {
    ...
    if (!isNaN(lat) && !isNaN(lng) && lat !== 0 && lng !== 0 && !locationInput.value.trim()) {
        // console.log('Prefilling location from coordinates:', lat, lng);
        ...
        if (typeof google !== 'undefined' && google.maps && google.maps.Geocoder) {
            ...
            geocoder.geocode({ location: latlng }, function(results, status) {
                if (status === 'OK' && results[0]) {
                    const address = results[0].formatted_address;
                    locationInput.value = address;
                    // console.log('Location prefilled with:', address);
                } else {
                    console.log('Geocoding failed:', status);
            ...
            // Fallback: show coordinates if Google Maps is not available
            locationInput.value = `${lat}, ${lng}`;
        }
    }
}
...
function initializeMapWithExistingCoordinates() {
    ...
    if (!isNaN(lat) && !isNaN(lng) && lat !== 0 && lng !== 0) {
        // console.log('Initializing map with existing coordinates:', lat, lng);
...
function initPartnerAutocomplete() {
    ...
    function updateMapAndFields(lat, lng, placeName) {
        ...
        if (partnerMap && marker) {
            ...
            partnerMap.setCenter(newPosition);
            partnerMap.setZoom(15); // Zoom in to show the location clearly
