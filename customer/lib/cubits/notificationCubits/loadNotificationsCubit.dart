...
class NotificationsCubit extends Cubit<NotificationsState> {
  ...
  void removeNotificationFromList(final dynamic id) {
    if (state is NotificationFetchSuccess) {
      final List notificationList =
          (state as NotificationFetchSuccess).notificationsList;
      ...
      emit(
        NotificationFetchSuccess(
          ...
          totalNotification:
              (state as NotificationFetchSuccess).totalNotification,
          loadMoreError: '',
        ),
      );
    }
  ...
  Future<void> fetchMoreNotifications() async {
    try {
      ...
      final NotificationFetchSuccess currentState =
          state as NotificationFetchSuccess;
      emit(
      ...
      final List<NotificationDataModel> oldList =
          currentState.notificationsList;
      ...
      emit(
        NotificationFetchSuccess(
          ...
          totalNotification: response['totalNotifications'],
          offset: (int.parse(currentState.offset) +
                  int.parse(UiUtils.limitOfAPIData))
              .toString(),
          isLoadingMoreData: false,
          loadMoreError: '',
        ),
