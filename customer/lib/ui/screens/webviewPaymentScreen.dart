...
class _WebviewPaymentScreenState extends State<WebviewPaymentScreen> {
    ...
    ..setNavigationDelegate(
      NavigationDelegate(
        ...
        onNavigationRequest: (final request) {
          final bool isPreventNavigation = pageRedirection(request.url);
  ...
  bool pageRedirection(final String url) {
    if (url.startsWith("${baseUrl}app_payment_status") ||
        url.startsWith("${baseUrl}app_paystack_payment_status") ||
        url.startsWith("${baseUrl}flutterwave_payment_status") ||
        url.startsWith(
            "${context.read<SystemSettingCubit>().paymentGatewaysSettings?.xenditWebsiteUrl}/payment-status") ||
        url.startsWith(
            '${context.read<SystemSettingCubit>().paymentGatewaysSettings?.flutterwaveWebsiteUrl}/payment-status') ||
        url.startsWith(
            '${context.read<SystemSettingCubit>().paymentGatewaysSettings?.paypalWebsiteUrl}/payment-status')) {
