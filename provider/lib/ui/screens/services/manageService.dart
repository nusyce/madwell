...
class ManageServiceState extends State<ManageService> {
  ...
  void _populateTranslatedControllers() {
    ...
    if (widget.service?.translatedFields?['title'] != null) {
      ...
      // Check if title is a Map (language code -> value) or a List/String
      if (titleData is Map) {
        for (var lang in languages) {
          final String code = lang.languageCode;
        ...
        // If it's a List, use it for the default language only
        if (languages.isNotEmpty) {
          final defaultLangCode = languages[0].languageCode;
          titleControllers[defaultLangCode]?.text = titleData.join(',');
        }
      } else {
        // If it's a String or other type, use it for the default language only
        if (languages.isNotEmpty) {
          final defaultLangCode = languages[0].languageCode;
    ...
    if (widget.service?.translatedFields?['description'] != null) {
      ...
      // Check if description is a Map (language code -> value) or a List/String
    ...
    if (widget.service?.translatedFields?['tags'] != null) {
      ...
      // Check if tags is a Map (language code -> value) or a List/String
      if (tagsData is Map) {
        for (var lang in languages) {
          final String code = lang.languageCode;
    ...
    if (widget.service?.translatedFields?['long_description'] != null) {
      ...
      // Check if long_description is a Map (language code -> value) or a List/String
    ...
    if (widget.service?.translatedFields?['faqs'] != null) {
      ...
      // Check if faqs is a Map (language code -> value) or a List
      if (faqsData is Map) {
        for (var lang in languages) {
          final String code = lang.languageCode;
        ...
        // If it's a List at top level, use it for the default language only
    ...
    if (widget.service?.translatedFields?['seo_description'] != null) {
      ...
      // Check if seo_description is a Map (language code -> value) or a List/String
    ...
    if (widget.service?.translatedFields?['seo_keywords'] != null) {
      ...
      // Check if seo_keywords is a Map (language code -> value) or a List/String
    ...
    if (widget.service?.translatedFields?['seo_schema_markup'] != null) {
      ...
      // Check if seo_schema_markup is a Map (language code -> value) or a List/String
  ...
  // Method to set HTML editor text with retry mechanism
  Future<void> _setHtmlEditorTextWithRetry(
    ...
    // Significantly longer delays - editor needs more time to initialize
    // Start with 2 seconds, then increase exponentially
    ...
    // Try to set text, catch any exceptions and retry
    try {
      controller.setText(text);
      // If successful, we're done
      return;
    } catch (e) {
      // Check if it's the "HTML editor is still loading" exception
      final errorMessage = e.toString().toLowerCase();
      if (errorMessage.contains('html editor is still loading') ||
          errorMessage.contains('please wait')) {
        // Editor not ready yet, retry with longer delay
        if (attempt < 9) {
          // Schedule next retry without blocking
          Future.delayed(
      ...
      // Silently handle other exceptions or if max attempts reached
    }
  }

  // Method to clear HTML editor with retry mechanism
  Future<void> _clearHtmlEditorWithRetry({int attempt = 0}) async {
    ...
    // Try to clear, catch any exceptions and retry
    try {
      controller.clear();
      // If successful, we're done
      return;
  ...
  Map<String, List<Map<String, String>>> getFaqData() {
    ...
    // First, ensure Form3 data is saved before getting it
    if (form3StateKey.currentState != null) {
      try {
    ...
    // Fallback: Get FAQ data from parent controllers if Form3 state is not accessible
  ...
  // Method to get SEO title data for API
  ...
  // Method to get SEO description data for API
  ...
  // Method to get SEO keywords data for API
  ...
  // Method to get SEO schema markup data for API
    ...
    if (isNext) {
      ...
      if (currIndex > 3 || form!.validate()) {
        ...
        if (currIndex < totalForms) {
          ...
          // Ensure FAQ data is saved if Form3 exists (even if we're not on that form)
          if (form3StateKey.currentState != null) {
            try {
              form3StateKey.currentState!.saveData();
            } catch (e) {
