        ...
        <div className="commanSec mt-12 flex flex-col items-start justify-center gap-6 w-full container mx-auto">
          ...
          <div className="filterSec flex flex-col gap-3 mt-4 w-full">
            ...
            <div className="hidden md:flex items-center gap-3 w-full">
              <div className="flex items-center justify-between gap-2 px-4 py-2 border rounded-md description_color flex-grow bg-white">
              ...
              {Array.isArray(categories) && categories.length > 0 && (
                <div className="flex items-center px-4 py-2 border rounded-md description_color min-w-[220px] bg-white">
                  <Select
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      {categories.map((cat) => {
                        const translatedCategoryName = cat?.translated_name ? cat?.translated_name : cat?.name;
                        return (
                          <SelectItem key={cat?.id || cat?.slug} value={String(cat?.id)}>
              ...
              <div className="flex items-center space-x-2 gap-2">
                ...
                <Select onValueChange={handleSortChange} value={sortOption}>
                  <SelectTrigger className="w-[200px] px-4 py-2 border rounded-md description_color focus:outline-none focus:ring-0 focus:ring-transparent">
                    <SelectValue placeholder="Select Filter" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="popularity">{t("popularity")}</SelectItem>
                    <SelectItem value="discount">{t("dicountHightoLow")}</SelectItem>
            ...
            <div className="md:hidden flex flex-col gap-3">
              <div className="flex items-center justify-between gap-2 px-4 py-2 border rounded-md description_color bg-white">
                <div className="flex items-center gap-2 w-full">
                  <FaSearch size={18} className="description_color" />
                  <input
                    type="text"
              ...
              {showMobileFilters && (
                <div className="w-full flex flex-col gap-3">
                  <div className="flex items-center px-4 py-2 border rounded-md description_color bg-white">
                    <Select
                      onValueChange={handleCategoryChange}
                      value={category || "all"}
                    >
                      ...
                      <SelectContent>
                        ...
                        {categories.map((cat) => (
                          <SelectItem key={cat?.id || cat?.slug} value={String(cat?.id)}>
                            {cat?.translated_name || cat?.name || cat?.title || cat?.slug}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  ...
                  <Select onValueChange={handleSortChange} value={sortOption}>
                    <SelectTrigger className="w-full px-4 py-2 border rounded-md description_color focus:outline-none focus:ring-0 focus:ring-transparent bg-white">
                      <SelectValue placeholder="Select Filter" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="popularity">{t("popularity")}</SelectItem>
        ...
        <div className="loadmore my-6 flex items-center justify-center">
          {currentProviders < totalProviders && (
            <button
              ...
              className={`${isLoading || isFetching
