...
const BookingDetails = () => {
  ...
  const createPaymentIntent = async (order_id, transactionID) => {
    ...
    const isZeroDecimal = zeroDecimalCurrencies.includes(stripeCurrencyCode?.toUpperCase());

    if (totalAdditionalCharges) {
      try {
        const paymentIntent = await stripeX.paymentIntents.create({
          amount: isZeroDecimal ? Math.round(totalAdditionalCharges) : Math.round(totalAdditionalCharges * 100), // Amount in cents
