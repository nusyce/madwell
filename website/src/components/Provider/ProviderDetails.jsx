...
const ProviderDetails = () => {
  ...
  const {
    ...
    queryKey: buildLanguageAwareKey(['provider', locationData?.lat, locationData?.lng, slug]),
    queryFn: async () => {
    ...
    enabled: !!locationData?.lat && !!locationData?.lng && !!slug && router.isReady,
    retry: 3,
    retryDelay: 1000,
  });
    ...
    onSuccess: (data) => {
      if (data?.error === false) {
        ...
        queryClient.invalidateQueries(
          buildLanguageAwareKey(['provider', locationData?.lat, locationData?.lng, slug])
        );
      } else {
  ...
  const handleChat = (e) => {
    ...
    try {
      dispatch(getChatData({
        ...
        partner_name: providerData?.translated_company_name ? providerData?.translated_company_name : providerData?.company_name,
  ...
  const isPreBookingChatAvailable = settings?.general_settings?.allow_pre_booking_chat === "1";
  const isProviderPreBookingChatAvailable = providerData?.pre_booking_chat === "1";
  ...
  const translatedCompanyName = providerData?.translated_company_name ? providerData?.translated_company_name : providerData?.company_name;
                  ...
                  <div className="light_bg_color rounded-lg overflow-hidden mt-4 relative p-5">
                    <div>
                      ...
                      <div className="photos grid grid-cols-3 gap-4 mt-6">
                        {providerData?.other_images?.slice(0, 4).map((image, index) => (
                          <div
