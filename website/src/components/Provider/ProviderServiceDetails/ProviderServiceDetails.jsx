...
const ProviderServiceDetails = () => {
  ...
  const limit = 5;
  const reviewLoadKey = providerId && serviceId ? `${providerId}::${serviceId}` : null;
  ...
  const {
    ...
    queryKey: buildLanguageAwareKey(['serviceDetails', locationData?.lat, locationData?.lng, serviceId, providerId]),
    queryFn: async () => {
    ...
    enabled: !!locationData?.lat && !!locationData?.lng && !!serviceId && !!providerId && router.isReady,
  });
  ...
  const translatedServiceName = serviceData?.translated_title || serviceData?.title;
  const translatedServiceDescription = serviceData?.translated_description || serviceData?.description;
  const translatedLongDescription = serviceData?.translated_long_description || serviceData?.long_description;
                    ...
                    <>
                      <span className="flex items-center gap-1">
                        ...
                        <span className="ml-1 text-sm flex items-center gap-1">
                          <span> {serviceData?.number_of_members_required}</span>{" "}
                          <span className="hidden lg:block"> {t("persons")}</span>
                        </span>
                      </span>
                      <span className="border border-gray-500 h-3"></span>
                    </>
                    ...
                    {serviceData?.other_images.slice(0, 6).map((image, index) => (
                      ...
                      >
                        <div className="relative rounded-md overflow-hidden">
                          ...
                          />
                          {index === 5 && serviceData?.other_images.length > 6 && (
