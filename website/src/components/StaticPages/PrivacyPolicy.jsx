...
const PrivacyPolicy = () => {
  ...
  const privacy_policy = settingsData?.settings;

  // Use optional chaining to avoid runtime error if privacy_policy is missing.
  const translatedPrivacyPolicy = privacy_policy?.translated_customer_privacy_policy
    ? privacy_policy.translated_customer_privacy_policy
    : privacy_policy?.customer_privacy_policy;
  ...
  return (
    <Layout>
      ...
      <section className="contact-us my-12 container mx-auto min-h-[50vh]">
        ...
        */}
        {translatedPrivacyPolicy ? (
          <RichTextContent content={translatedPrivacyPolicy} />
        ) : (
          <div className='w-full h-[60vh] flex items-center justify-center'>
          <NoDataFound
              title={t("noDataTitle") || "No information available"}
              desc={t("noDataDescription") || "Privacy policy content is not available right now."}
            />
          </div>
        )}
      </section>
