 ...
 * 
 * This component handles provider registration with phone verification flow:
 * 1. Phone number input with country code selection
 * 2. OTP verification using verifyProviderApi (Firebase or SMS Gateway)
 * 3. Registration form completion using registerProviderApi
 * 4. Success message with app download links
 * 
 * Complete API Flow:
 * 1. verifyProviderApi - Verify phone and get authentication mode
 *    - Message Code 101: Mobile already registered and Active (show error)
 *    - Message Code 102: Proceed for OTP (continue registration)
 *    - Message Code 103: Mobile number is Deactive (show error)
 * 2. verifyProviderOtpApi/resendProviderOtpApi - OTP verification (SMS Gateway mode)
 * 3. registerProviderApi - Final registration with user details
 * 
 * Authentication Flow:
...
import { MdCheckCircle, MdPhoneAndroid, MdComputer, MdAppRegistration } from "react-icons/md";
...
const RegisterAsProviderModal = ({ isOpen, onClose }) => {
    ...
    const handleContinueToOtp = async () => {
        ...
        // Call verifyProviderApi to verify phone number and check authentication mode
        // Expected response structure:
        // {
        //   error: false,
        //   message_code: "101" | "102" | "103", // specific message codes
        //   authentication_mode: "firebase" | "sms_gateway",
        //   uid: "unique_id", // if available
        //   message: "success message"
        // }
        // 
        // Message Code Meanings:
        // - "101": Mobile number already registered and Active (show error)
        // - "102": Proceed for OTP (continue with registration)
        // - "103": Mobile number is Deactive (show error)
        setLoading(true);
        ...
        try {
            ...
            // Check message code and handle accordingly
            const messageCode = response?.message_code;
            setMessageCode(messageCode);

            // Handle different message codes
            if (messageCode === "101") {
                // Mobile number already registered and Active
                showErrorMessage(t("mobileNumberAlreadyRegisteredAndActive"));
                setLoading(false);
            } else if (messageCode === "102") {
                // Proceed for OTP - check authentication mode
                if (response?.authentication_mode === "firebase") {
                    // Firebase mode: send OTP via Firebase (generate new reCAPTCHA)
                    await sendOtpViaFirebase(phoneNumberWithoutDialCode, false);
                    setSmsMethod("firebase");
                } else {
                    // SMS Gateway mode: proceed to OTP screen (OTP already sent by API)
                    setLoading(false);
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            if (smsMethod === "firebase") {
                ...
                // - uid: unique identifier from verifyProviderApi response (if available)
                // - otp: 6-digit OTP code


                const response = await verifyProviderOtpApi({
                    mobile: phoneNumberWithoutDialCode,
                    country_code: countryCode,
                    otp: otp
                });
    ...
    const handleFinalRegistration = async () => {
        ...
        try {
            // Get phone number without country code for API
            const phoneNumberWithoutDialCode = formatPhoneNumber();
                        ...
                        <div className="flex-1">
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-start justify-between w-full gap-1 mb-6">
                                        ...
                                        <div className="text-start">
                                            <h2 className="text-2xl font-bold mb-2">{t("registerAsProvider")}</h2>
                                            <p className="description_color">{t("enterYourNumberToGetVerified")}</p>
                                        </div>
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-3 font-semibold rounded-md transition-colors ${phone && !loading
                            ...
                            {currentStep === 2 && (
                                <div className="space-y-6">
                                    <div className="flex items-start justify-between w-full gap-1 mb-6">
                                        <div className="text-start">
                                            <h2 className="text-2xl font-bold mb-2">{t("verifyOTP") || "Verify OTP"}</h2>
                                            <p className="description_color max-w-md">
                                                {t("weJustSentYouSixDigitCode")}
                                                <br />
                                                <span className="font-bold" style={{ direction: "ltr", unicodeBidi: "isolate" }}>
                                                    {showFullPhoneNumber}
                                                </span>
                                            </p>
                                        </div>
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-2 font-semibold rounded-md transition-colors ${resendAvailable
                                            ? "primary_bg_color text-white hover:bg-blue-700"
                                            : "bg-gray-300 text-gray-500 cursor-not-allowed"
                                            }`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-3 font-semibold rounded-md transition-colors ${otp.length === 6 && !otpLoading
                                            ? "primary_bg_color text-white hover:bg-blue-700"
                                            : "bg-gray-300 text-gray-500 cursor-not-allowed"
                                            }`}
                                    >
                                        {otpLoading ? t("processing") : t("verifyOTP") || "Verify OTP"}
                                    </button>
                                </div>
                            )}
                            ...
                            {currentStep === 3 && (
                                <div className="space-y-6">
                                    <div className="flex items-start justify-between w-full gap-1 mb-6">
                                        <div className="text-start">
                                            <h2 className="text-2xl font-bold mb-2">{t("completeRegistration") || "Complete Registration"}</h2>
                                            <p className="description_color">{t("fillInYourDetails") || "Please fill in your details to complete registration"}</p>
                                        </div>
                                        {/* Right Side - Step Indicator */}
                                        {renderStepIndicator()}
                                    </div>
                            ...
                            {currentStep === 4 && registrationSuccess && (
                                <div className="space-y-8 text-center">
                                    ...
                                    <div className="space-y-4 max-w-md mx-auto">
                                        ...
                                        {/* Mobile Apps Section */}
                                        <div className="text-center">
                                            <p className="text-sm text-gray-500 mb-3">{t("downloadOurMobileProviderApps") || "Download our mobile provider apps"}</p>
