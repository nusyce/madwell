...
import { setCartData, clearCart } from '@/redux/reducers/cartSlice';
...
export function useCart({ order_id = null, enabled = null } = {}) {
  ...
  const { 
    ...
    staleTime: 1000 * 30, // Consider data stale after 30 seconds (cart changes more frequently)
    ...
    onSuccess: (response) => {
      ...
      if (response?.error === false) {
        const cartData = response.data?.cart_data || response.data?.reorder_data;
  ...
  const invalidateCart = async () => {
    await queryClient.invalidateQueries(buildLanguageAwareKey(['cart']));
    await refetchCart();
  };
